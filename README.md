...
<hr>
<h4>3. Записываем функцию, которая вычисляет разницу между временем окончания отсчета в мс - функция getTimeRemaining</h4>. Эта функция принимает один аргумент - endtime, который в дальнейшем будут использовать и другие функции. Функция getTimeRemaining также вычисляет сколько целых дней, часов, минут содержится в этой разнице, и остаток секунд.
 ...
 <hr>
 <h4>4. Теперь нам нужно вставить полученные дни, часы, минуты и секунды в верстку</h4> И сделать это динамически с обновлением каждые 100 мс (1с). За это отвечает функция setClock, которая принимает 2 аргумента: id и все тот же endtime.<br> Запишем в локальные переменные участвующие DOM элементы и временной интревал обновления таймера:
 
 
<img width="431" alt="Screenshot 2023-05-23 at 16 52 18" src="https://github.com/polousova/countdown/assets/113363158/87310c67-e625-4643-923d-6940b9af3844">
